...
class Phase2Agent:
    ...
    def _execute_meta_action(self, action: MetaAction, metrics: Dict[str, float]) -> float:
        ...
        pre_action_confidence = metrics.get('val_confidence', 0.5)
        reward = 0.0
        ...
        if action == MetaAction.CONTINUE_TRAINING:
        ...
        elif action == MetaAction.INCREASE_BATCH_SIZE:
            print("    Executing: Increasing batch size")
            old_batch_size = getattr(self, 'batch_size', 32)
            new_batch_size = min(old_batch_size * 2, 512)  # Cap at 512
            ...
            if hasattr(self, 'train_loader') and self.train_loader is not None:
                try:
                    ...
                    print(f"      Failed to update train_loader: {e}")
            print(f"      Batch size: {old_batch_size} -> {new_batch_size}")
            # Calculate performance-based reward
            reward = self._calculate_performance_reward(action, pre_action_loss, pre_action_confidence)
        
        elif action == MetaAction.DECREASE_BATCH_SIZE:
            print("    Executing: Decreasing batch size")
            old_batch_size = getattr(self, 'batch_size', 32)
            new_batch_size = max(old_batch_size // 2, 4)  # Minimum of 4
            self.batch_size = new_batch_size
            # Update data loader if it exists
            if hasattr(self, 'train_loader') and self.train_loader is not None:
                try:
        ...
        elif action == MetaAction.SWITCH_OPTIMIZER:
            print("    Executing: Switching optimizer")
            # Simple optimizer switching logic
            current_optimizer_type = type(self.optimizer).__name__
            if 'Adam' in current_optimizer_type:
                # Switch to SGD
                self.optimizer = torch.optim.SGD(self.model.parameters(), lr=self.optimizer.param_groups[0]['lr'], momentum=0.9)
                print(f"      Switched from {current_optimizer_type} to SGD")
            else:
                # Switch to Adam
                self.optimizer = torch.optim.Adam(self.model.parameters(), lr=self.optimizer.param_groups[0]['lr'])
                print(f"      Switched from {current_optimizer_type} to Adam")
            reward = 0.1  # Moderate reward for optimizer change
        
        elif action == MetaAction.GRADIENT_CLIPPING:
            print("    Executing: Enabling gradient clipping")
            self.gradient_clip_value = 1.0  # Set gradient clipping value
            print(f"      Gradient clipping enabled with value: {self.gradient_clip_value}")
            reward = 0.05  # Small reward for enabling gradient clipping
        
        elif action == MetaAction.CURRICULUM_ADVANCE:
            print("    Executing: Advancing curriculum")
            if hasattr(self, 'curriculum'):
                try:
                    ...
                    print(f"      Curriculum level: {old_level} -> {new_level}")
                    reward = 0.2  # Good reward for curriculum advancement
                except Exception as e:
                    print(f"      Failed to advance curriculum: {e}")
                    reward = 0.0
            else:
                print("      No curriculum available")
                reward = 0.0
        
        elif action == MetaAction.EMERGENCY_STOP:
            print("    Executing: Emergency stop")
            self.emergency_stop_flag = True
            reward = -0.1  # Negative reward for emergency stop
        
        return reward
    
    def _calculate_performance_reward(self, action: MetaAction, pre_loss: float, pre_confidence: float) -> float:
        """Calculate reward based on actual performance impact of the action."""
        try:
            # Take a quick validation sample to measure immediate impact
            current_metrics = self.validate()
            post_loss = current_metrics.get('val_loss', pre_loss)
            post_confidence = current_metrics.get('val_confidence', pre_confidence)
            
            # Calculate improvement
            loss_improvement = pre_loss - post_loss
            confidence_improvement = post_confidence - pre_confidence
            
            # Base reward for making the action
            base_reward = 0.1
            
            # Performance-based adjustment
            performance_reward = loss_improvement * 2.0 + confidence_improvement * 1.0
            
            # Ensure reward is reasonable
            total_reward = base_reward + np.clip(performance_reward, -0.5, 0.5)
            
            print(f"      Performance reward: base={base_reward:.3f}, perf={performance_reward:.3f}, total={total_reward:.3f}")
            return total_reward
            
        except Exception as e:
            print(f"      Failed to calculate performance reward: {e}")
