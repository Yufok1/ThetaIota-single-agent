...
TIMEOUT_SEC = None  # No timeout for agent responses
RETRIES = 3
RETRY_DELAY_SEC = 1.0
...
def smart_input(prompt="> "):
    """Smart input function that handles multi-line pastes."""
    import sys
    import time
    import os
    ...
    try:
        ...
        # Check for additional lines (indicating a paste operation)
        if os.name == 'nt':  # Windows
            try:
                import msvcrt
                # Check immediately for more input
                time.sleep(0.001)  # Very brief pause for input to buffer
                while msvcrt.kbhit():
                    try:
                        line = input()
                        lines.append(line)
                        time.sleep(0.001)  # Brief pause between reads
                    except EOFError:
                        break
            ...
            try:
                ...
                if hasattr(sys.stdin, 'fileno'):
                    # Check for more input with very short timeout
