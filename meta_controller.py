...
class MetaController:
    ...
    def evaluate_response(self, response: str) -> Tuple[bool, Optional[str]]:
        """
        Evaluate if a response is complete and provide feedback.
        ...
        if any(phrase in response.lower() for phrase in [
            ...
            "therefore", "in summary", "the answer is"
        ]):
            complete = True
            feedback = "Response appears complete with conclusion"
        
        # Check for length - very short responses might be incomplete
        elif len(response.strip()) < 50:
            complete = False
            feedback = "Response seems too brief, please expand"
        
        # Check for repetition - if same content repeats, it's complete
        elif self._detect_repetition(response):
            complete = True
            feedback = "Detected repetitive content, marking as complete"
        
        # Default case for reasonable length responses
        elif len(response.strip()) > 200:
            complete = True
            feedback = "Response has sufficient detail"
    ...
    def _detect_repetition(self, text: str) -> bool:
        ...
        # Check if multiple sentences are very similar
